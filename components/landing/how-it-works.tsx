...
const HowItWorks = () => {
	return (
		<section className="py-20 px-4 bg-white font-['Inter',sans-serif]">
			{/* Header Section */}
			<div className='max-w-6xl mx-auto text-center mb-16'>
				<div className='inline-block px-4 py-1 mb-6 text-sm font-medium text-gray-600 border border-gray-200 rounded-full'>
					How it Works
				</div>
			...
			<div className='relative max-w-6xl mx-auto mb-20'>
				...
				<div className='absolute top-1/2 left-0 w-full h-0.5 bg-gray-300 -translate-y-1/2 hidden md:block z-0' />

				<div className='grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10'>
					{steps.map((step, index) => (
						<div
							key={index}
							className='relative bg-white border border-gray-200 rounded-3xl p-10  hover:shadow-md transition-shadow duration-300'>
							{/* Top Decorative Dot */}
							<div className='absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full flex items-center justify-center bg-white'>
								<div
							...
							<div className='text-8xl font-bold text-gray-100 mb-6 select-none'>
								{step.number}
							</div>
							...
							<div className='flex gap-2 items-center'>
								...
								/>
								<span className='h-1 flex-1 bg-gray-200 rounded-full' />
							</div>
						</div>
					))}
				</div>
